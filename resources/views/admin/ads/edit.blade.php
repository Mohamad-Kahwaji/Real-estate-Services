...
.form-card {
    ...
    box-shadow:0 4px 24px rgba(var(--role-accent-rgb),.08); padding:28px 32px; max-width:760px;
}
.form-card-title {
...
.form-card-title i { color:var(--role-accent); font-size:18px; }
...
.f-input:focus { border-color:var(--role-accent); background:#fff; box-shadow:0 0 0 4px rgba(var(--role-accent-rgb),.1); }
.img-preview-wrap {
    border:2px dashed #d4d5ff; border-radius:14px; padding:16px;
    text-align:center; cursor:pointer; transition:.2s; background:#faf9fc; position:relative;
}
.img-preview-wrap:hover { border-color:var(--role-accent); background:#f0f1ff; }
.current-img { max-height:180px; border-radius:10px; margin:0 auto; display:block; }
.btn-save {
    padding:12px 28px; border-radius:12px; border:none;
    background:linear-gradient(135deg,var(--role-accent),var(--role-accent-light));
    color:#fff; font-size:14px; font-weight:700; cursor:pointer;
    box-shadow:0 4px 16px rgba(var(--role-accent-rgb),.35); transition:.2s;
    display:inline-flex; align-items:center; gap:8px;
}
.btn-save:hover { transform:translateY(-1px); }
.toggle-switch { display:flex; align-items:center; gap:12px; }
.toggle-switch input[type=checkbox] { width:42px; height:22px; accent-color:var(--role-accent); cursor:pointer; }
</style>

<div style="margin-bottom:20px;">
    <a href="{{ route('ads.index') }}" style="color:var(--role-accent);font-size:13px;font-weight:700;text-decoration:none;display:inline-flex;align-items:center;gap:6px;">
...
<div class="form-card">
    <div class="form-card-title">
        <i class="ri ri-edit-line"></i> {{ __('app.edit_ad') }}
    </div>
        ...
        <div class="row g-4">
            ...
            <div class="col-12">
                ...
                <textarea name="description" class="f-input" rows="3"
                          style="resize:vertical;">{{ old('description', $ad->description) }}</textarea>
            </div>
            ...
            <div class="col-12">
                ...
                <div style="position:relative;">
                    ...
                    <input type="url" name="link" class="f-input"
            ...
            <div class="col-12">
                ...
                <div class="img-preview-wrap" onclick="document.getElementById('imgInput').click()">
                    @if($ad->image)
                        <img id="imgPreview" class="current-img"
                             src="{{ $ad->image_url }}" alt="{{ $ad->title }}">
                        <div style="font-size:11px;color:#97939e;margin-top:8px;">{{ __('app.click_to_change') }}</div>
                    @else
                        <img id="imgPreview" style="display:none;max-height:180px;border-radius:10px;" src="" alt="">
                        <div id="imgPlaceholder">
                            <i class="ri ri-image-add-line" style="font-size:36px;color:var(--role-accent-light);display:block;margin-bottom:8px;"></i>
            ...
            <div class="col-12">
                ...
                <div class="toggle-switch">
                    <input type="checkbox" name="is_active" id="activeToggle"
